...
export class MapComponentComponent implements OnInit {
  @Input() localisation: Localisation = {
    latitude: 53.13333,
    longtitude: 23.16433,
    localisationName: 'Bialystok',
  };
  @Input() isOpenInNewPostForm = false;
  @Output() selectedCoordinates: EventEmitter<Localisation> =
    new EventEmitter();
  ...
  public ngOnInit(): void {
    this.map = L.map('map').setView(
      [this.localisation.latitude, this.localisation.longtitude],
      12,
    );
    this.popup = L.popup();
    ...
    if (!this.isOpenInNewPostForm) {
      this.marker = L.marker([
        ...
        this.localisation.longtitude,
      ]).addTo(this.map);
      this.marker
        .bindPopup(
          `<b>${this.localisation.latitude}, ${this.localisation.longtitude}</b><br>${this.localisation.localisationName}.`,
        )
        .openPopup();
    }
    if (this.isOpenInNewPostForm) {
  ...
  private selectLocalisation = (event: any) => {
    ...
    this.geocodeService
      ...
      .run((error, result) => {
        ...
        const localisationObj = new Localisation({
    ...
    this.popup
      ...
      .setContent(
        `Selected ${event.latlng.lat.toString()}, ${event.latlng.lng.toString()}`,
      )
      .openOn(this.map);
  };
}
